...
-   Given a ***KxN data matrix*** containing ***K*** variables (probably correlated measurements) in ***N*** samples (objects/individuals...)

-   PCA transforms the variables into ***K*** new components that
...
![Plot title. ](images/AMV4Omics-Xaringan_insertimage_20.png)
...
-   These new components are constructed in such a way that *they have decreasing explanatory power*: each component explains more than the next (less

) variability in the data.

-  Practically, in most cases, a few components (two, three, etc.) are enough to summarize the majority of the information contained in the original variables.

-  The principal components are calculated by projecting the points onto a reduced number of axes, thus forming a plane or a hyperplane that minimizes the Euclidean distance between the points and their projections.
]

.pull-right[
![Plot title. ](images/AMV4Omics-Xaringan_insertimage_21.png)
...
-   The output of a PCA analysis includes

  -   A summary of the variability explained by each of the principal components and a plot (scree plot) that shows how it decreases with the component number.

  -   Scores: New coordinates of samples on the PC axes

  -   Loadings: Coordinates of variables on the PC axes
]

.pull-right[
![Plot title. ](images/AMV4Omics-Xaringan_insertimage_22.png)
]
...
-   Classical example: Iris data
  -   First two principal components (from the four original variables) can separate three species

]

.pull-right[
![Plot title. ](images/AMV4Omics-Xaringan_insertimage_23.png)
[Link to code](https://gist.github.com/chriddyp/7c6c43682a8f57a1f999)
]
...
- The second steps show aspects such as data transformation and projection using PCA.
